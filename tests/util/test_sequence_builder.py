...
def test_length():
    assert all(len(seq) == 4 for seq in SequenceBuilder(4, options=(0, 1, 2), i_constraints={0: is_even}))  # type: ignore[var-annotated]


def test_first_constraint():
    assert all(is_even(seq[0]) for seq in SequenceBuilder(4, options=(0, 1, 2), i_constraints={0: is_even}))  # type: ignore[var-annotated]
...
def test_prev_curr(options):
    for cycle in SequenceBuilder(5, options=options, curr_prev_constraint={-1: even_odd_interchange}, loop=True):  # type: ignore[var-annotated]
...
def test_prefix(options, parallel):
    prefix = options[:3]
    assert all(seq[:len(prefix)] == prefix for seq in SequenceBuilder(5, options=options, prefix=prefix, parallel=parallel))  # type: ignore[var-annotated]


@pytest.mark.parametrize('parallel', [False, True])
def test_unique(options, parallel):
    assert all(len(seq) == len(set(seq)) for seq in SequenceBuilder(4, options=options, unique_key=identity, parallel=parallel))  # type: ignore[var-annotated]
    assert any(len(seq) != len(set(seq)) for seq in SequenceBuilder(4, options=options, parallel=parallel))  # type: ignore[var-annotated]
